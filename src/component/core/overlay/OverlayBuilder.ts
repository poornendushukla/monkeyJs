...
class OverlayBuilder implements IOverlay {
  ...
  update(domRect: DOMRect) {
    ...
    let maskPath: string = this.createSvgPathWithCutout(domRect);
  ...
  mount(): void {
    ...
    try {
      ...
      console.error("Failed to mount overlay");
  ...
  unmount(): void {
    ...
    try {
      ...
      console.error("Failed to unmount Overlay");
  ...
  distroy(): void {
    try {
      ...
      console.error("Failed to distroy overlay.");
